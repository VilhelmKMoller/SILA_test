...
)  # noqa: S104, possible bind to all interfaces
...
def main(
    ...
    device_info_path: str = Option(
        False,
        "--device-info-path",
        help="path to device info jsonld file, including file name",
    ),
    serial_port: Optional[str] = Option(
        None, "--serial-port", help="The serial / COM port of the hardware"
    ),
    simulation: bool = Option(
        False, "--simulation", help="Start Server in simulation mode"
    ),
):
    # validate parameters
    ...
    private_key = (
        Path(private_key_file).read_bytes() if private_key_file is not None else None
    )
    ca_for_discovery = (
        Path(ca_file_for_discovery).read_bytes()
