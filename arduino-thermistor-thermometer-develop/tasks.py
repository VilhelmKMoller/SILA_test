...
SOURCE_DIR = ROOT_DIR.joinpath("arduino_thermistor_thermometer")
...
PYTHON_VERSION = 3.9
CI_PROJECT_NAME = "arduino-thermistor-thermometer"
CI_REGISTRY_IMAGE = "registry.gitlab.com/https://gitlab.com/opensourcelab/devices/temp-ctrl/arduino-thermistor-thermometer"
...
def docker_test(_c, python_version=PYTHON_VERSION):
    ...
    """
    volume_mount = (
        f"--volume {BIN_DIR}:/arduino_thermistor_thermometer/bin/ --volume {ROOT_DIR}:/arduino_thermistor_thermometer:rw"
    )
    registry_path = _get_registry_path_str(python_version)
    pytest_arg = f"pytest -v --cov-report xml:/arduino_thermistor_thermometer/bin/coverage.xml {TEST_DIR}"
    Path(BIN_DIR).mkdir(parents=True, exist_ok=True)
